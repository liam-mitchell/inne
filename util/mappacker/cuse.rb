...
def log_exception(e, msg)
  str1 = "ERROR! Failed to #{$installed ? 'uninstall' : 'install'} CUSE :("
...
def install
  print "\n┏━━━ Installing N++ CUSE\n┃\n"
  patch
  change_levels(true)   if CHANGE_LEVELS
  change_texts(true)    if CHANGE_TEXTS
  change_controls(true) if CHANGE_CONTROLS
  change_nprofile(true) if CHANGE_NPROFILE
  change_author(true)   if CHANGE_AUTHOR
  puts "┃\n┗━━━ Installed CUSE successfully!\n\n"
  dialog("N++ Mappack", "Installed N++ CUSE successfully!")
end

def uninstall
  print "\n┏━━━ Uninstalling N++ CUSE\n┃\n"
  depatch
  change_levels(false)   if CHANGE_LEVELS
  change_texts(false)    if CHANGE_TEXTS
  change_controls(false) if CHANGE_CONTROLS
  change_nprofile(false) if CHANGE_NPROFILE
  change_author(false)   if CHANGE_AUTHOR
  puts "┃\n┗━━━ Uninstalled CUSE successfully!\n\n"
  dialog("N++ Mappack", "Uninstalled N++ CUSE successfully!")
end

str1 = " N++ CUSE (Eddy, 2023) "
str2 = " Custom Userlevel Search Engine "
